...
populateTaxonomy config = do
  ...
  case resp of
    ...
    Right res -> do
      ...
      BLC.writeFile ("ebird-taxonomy-" <> show currTime <> ".csv") res
...
copyQ :: PG.Query
copyQ = "COPY taxonomy(scientific_name, common_name, species_code, category, taxonomy_order, common_name_code, scientific_name_code, banding_codes, order_name, family_common_name, family_scientific_name, report_as, extinct, extinct_year, family_code) FROM STDIN DELIMITER ',' CSV HEADER;"
